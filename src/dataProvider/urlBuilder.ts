...
export const parseFilters = (filter : Array<string>, defaultListOp : PostgRestOperator) => {
  let result = {};
  Object.keys(filter).forEach(function (key) {
    ...
    const operation : PostgRestOperator = 
      splitKey.length == 2 ? splitKey[1] as PostgRestOperator : defaultListOp;
...
export const getPrimaryKey = (resource: string, primaryKeys: Map<string, PrimaryKey>) => {
...
export const getQuery = (primaryKey: PrimaryKey, ids: Identifier | Array<Identifier>, resource: string): string => {
...
export const getOrderBy = (field: string, order: string, primaryKey: PrimaryKey) => {
  if (field == 'id') {
